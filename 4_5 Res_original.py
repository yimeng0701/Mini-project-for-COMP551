...
# original image+ 240 filter + image augment + ResNet+ decaying learning rate
...
total_iterations = 0
break_signal=0
b_x=torch.zeros((BATCH_SIZE,1,64,64))
Modename='ResNet3-19（RES2）.pkl'

###########################################################################################################
#loading data
if TRY :
    train_x=np.zeros((1000,64*64))
    Valid_Bath_Size = 20
else: train_x=np.zeros((50000,64*64))

i=0

#plz change the name of the file accordingly; the 2GB original data
filename1="cs.mcgill.ca/~ksinha4/datasets/kaggle/train_x.csv"                                         #load training data
...
train_x=train_data[:,:-1].reshape(-1,64,64)
...
valid_x=valid_data[:,:-1].reshape(-1,64,64)
...
class ResNet(nn.Module):
    def __init__(self,block,layers,num_classes=10):
        ...
        self.in_channels = 16
        self.layer1=self.make_layer(block,16,blocks=layers[0])              #64*64
        self.layer2=self.make_layer(block,32,blocks=layers[1],stride=2)     #32*32
        self.layer3=self.make_layer(block,64,blocks=layers[2],stride=2)     #16*16
        self.layer4 = self.make_layer(block,128,blocks=layers[3],stride=2)  #8*8
        self.layer5 = self.make_layer(block,256,blocks=layers[4], stride=2) #4*4
...
transform = tfs.Compose([         
    ...
    tfs.RandomResizedCrop(64,scale=(0.8,1)),   #resize to 64*64
